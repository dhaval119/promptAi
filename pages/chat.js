...
export default function Chat() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.aiText) {
        ...
        setMessages(prev => [...prev, { sender: 'ai', text: 'Error generating prompt. Please try again.' }]);
      }
    } catch (err) {
  ...
  return (
    <>
      <Head>
        <title>AI Chat - PromptMagic</title>
      </Head>
      ...
      <div className="desktop">
        ...
        <aside className={`sidebar ${sidebarHidden ? 'hide' : ''}`}>
          <nav className="sidebar-nav">
            ...
            <ul className="sidebar-list">
              <li className="sidebar-item" style={{ opacity: 0.5, color: '#fff', fontSize: 14 }}>
                Chat History Here
              </li>
            </ul>
          </nav>
        ...
        <main className={`main-content ${sidebarHidden ? 'no-sidebar' : ''} ${isLanding ? 'view-landing' : 'view-chat'}`}>
          {isLanding && (
            <div className="landing-content">
              <h1 className="main-heading">Where should we begin?</h1>
              <div className="suggestion-buttons">
                <button className="suggestion-button" onClick={() => quickStart('Leave application email')}>
                  <span>Leave application email</span>
                  <img src="/assets/arrow.png" className="suggestion-arrow" alt="" />
                </button>
                <button className="suggestion-button" onClick={() => quickStart('Professional resume for')}>
                  <span>Professional resume for</span>
                  <img src="/assets/arrow.png" className="suggestion-arrow" alt="" />
                </button>
                <button className="suggestion-button" onClick={() => quickStart('Make website for Store')}>
              ...
              m.sender === 'user' ? (
                ...
                <section key={i} className="ai-group">
                  ...
                  <div className="copy-container" onClick={() => copyPrompt(m.text)}>
                    ...
                    <img src="/assets/copy.png" className="copy-icon" alt="copy" />
                  </div>
                </section>
              )
            ))}
      ...
      <style jsx>{`
        * { box-sizing: border-box; }
        ...
        .sidebar-nav {
          position: absolute; top: 120px; left: 25px; right: 10px; bottom: 20px;
          overflow-y: auto;
        }
        .new-chat {
          color: #fff; font-weight: 700; font-size: 18px;
          background: none; border: none; cursor: pointer;
          margin-bottom: 40px; text-align: left; padding: 0;
        }
        .chats-heading {
          color: #fff; opacity: 0.75; font-weight: 700;
        ...
        .view-landing .chat-input-form {
          ...
          width: var(--content-width); max-width: 90%;
        }
        ...
        .landing-content {
          position: absolute; top: calc(50% - 150px); left: 50%;
          transform: translateX(-50%);
          width: var(--content-width); text-align: center;
        }
        .main-heading {
        ...
        .suggestion-button {
          width: 32%; min-width: 180px; height: 36px;
          background: #000; border: 1px solid #fff; border-radius: 50px;
          color: #fff; font-weight: 600; font-size: 13px;
          display: flex; align-items: center; justify-content: space-between;
          cursor: pointer; padding: 0 15px;
        }
        .suggestion-button:hover { background: #fff; color: #000; }
        .suggestion-arrow { width: 12px; height: 12px; }
        ...
        .ai-prompt {
          font-weight: 400; color: #ececec; font-size: 15px;
          line-height: 1.5; background-color: #111111;
          padding: 20px; border-radius: 12px; border: 1px solid #222;
        ...
        .chat-input {
          ...
          background: #0e0e0e; border-radius: 50px; border: none;
          padding: 0 65px 0 35px; font-size: 18px; color: #fff; outline: none;
        }
        .chat-submit-button {
        ...
        @media (max-width: 768px) {
          .sidebar { width: 220px; }
          .nav-pill { right: 12px; gap: 12px; padding: 5px 14px; }
          .nav-pill a { font-size: 13px; }
          .main-heading { font-size: 28px; white-space: normal; }
          .suggestion-button { width: 100%; min-width: unset; }
          .conversation-thread { padding: 100px 16px 100px; }
          .view-chat .chat-input-form { width: 95%; bottom: 16px; }
        }
      `}</style>
    </>
  );
